...
// zp-flash-half：2026-09-22 复核后标为 expired —— 官方 notice 证实它就是
// 「GLM Coding Plan 夜间畅用」活动（9/3–9/20，已于 9/20 结束），
// 且本插件另有 zp-night-free 一条完整记录同一活动 ⇒ 该条属重复记录，故撤下。
// 状态因此由 unknown 变为 done（expired 的判定优先于「无截止日」）。
eq('已撤下的促销判为已结束（expired 优先于无截止日）', byId['zp-flash-half'].state, 'done');
truthy('已结束的活动仍保留在列表中（不删除）', camps.some((c) => c.state === 'done'));

// ⚠️ 覆盖缺口（2026-09-22 记录）：撤下 zp-flash-half 后，**已无任何活动缺截止日**，
// 「无截止日 → unknown」这条分支失去了真实数据覆盖。该分支不能少 ——
// 少了它，未公布截止日的促销会被判成 live，等于把「不确定」说成「确定」。
// 故改为静态断言守住它，防未来重构静默删掉。
truthy(
  'campaigns() 仍保留「无截止日 → unknown」分支',
  fs.readFileSync(PLUGIN, 'utf8').includes('state = "unknown"')
);
...
const must = [
  // 2026-09-22：板块标题由「模型使用时钟」更名为「模型优惠时钟」（用户指定），
  // 断言同步更新 —— 否则改名后这里会红，而红的是「测试没跟上」而不是「代码坏了」。
  ['标题', '模型优惠时钟'],
  ['此刻推荐区块', '此刻推荐'],
  ['热力带区块', '全天成本热力带'],
  ['调度建议区块', '等一等更省'],
  ['限时活动区块', '限时活动'],
  // 2026-09-22 新增的第六个区块（一次性买断的资源包，独立于时段口径）
  ['资源包特惠区块', '资源包特惠'],
  ['溯源区块', '数据溯源'],
  ['数据版本', '2026.09.21'],
  ['已结束状态可见', '已结束'],
